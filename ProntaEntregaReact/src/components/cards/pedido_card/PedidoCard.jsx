...
const PedidoCard = forwardRef(
  ({ productDefault, user, stock, stocksDisponibles }, ref) => {
    ...
    const handleFetchProducts = (id_stock, id_categoria) => {
      fetchData(`/producto/categoria/${id_categoria}/`, token)
        .then((result) => {
          setProducts(result);
        })
        .catch((error) => {
    ...
    const handleCardSelection = (key) => {
      ...
      setObras((prevObras) => {
        const selectedObra = prevObras.find(
          (obra) => obra.id_obra.id_obra === key,
        );
        const remainingObras = prevObras.filter(
          (obra) => obra.id_obra.id_obra !== key,
        );
        return selectedObra
          ? [selectedObra, ...remainingObras]
          : remainingObras;
      });
    };

    const handleSelectAllObras = () => {
      const allObrasIds = orderedObras
        .filter((obra) => obra.id_obra.id_obra !== pedidoForm.id_obra)
        .map((obra) => obra.id_obra.id_obra);
      const allSelected = allObrasIds.every((id) =>
        pedidoForm.obras.includes(id),
      );
    ...
    const validateForm = (form) => {
      ...
      if (
        form.fechainicio === "" ||
        isNaN(new Date(form.fechainicio).getTime())
      ) {
        formIsValid = false;
        ...
        formIsValid = false;
        errors.fechainicio =
          "Fecha de inicio no puede ser menor a la fecha de hoy";
      }

      if (
        form.fechavencimiento === "" ||
        isNaN(new Date(form.fechavencimiento).getTime())
      ) {
        formIsValid = false;
        ...
        errors.fechavencimiento =
          "Fecha de vencimiento no puede ser menor a la fecha de hoy";
      }
          ...
          <Form>
            <Row className="g-0">
              <Col xs={12} md={5} style={{ marginRight: "2rem" }}>
                <h2>
                  {productDefault
                    ? `Crear Pedido de ${productDefault.nombre}`
                    : "Crear Pedido"}
                </h2>
                ...
                <OverlayTrigger
                  ...
                  overlay={
                    <Tooltip
                      style={{ fontSize: "100%" }}
                      id="tooltip-fecha-inicio"
                    >
                      El pedido se hará visible este día
                    </Tooltip>
                  }
                >
                ...
                <OverlayTrigger
                  ...
                  overlay={
                    <Tooltip
                      ...
                      id="tooltip-fecha-inicio"
                    >
                      El pedido dejara de ser visible este día
                    </Tooltip>
                  }
                >
                  <Form.Group className="mb-2" controlId="formBasicFechaFin">
                    ...
                    <Form.Control
                      ...
                      onBlur={handleInputChange}
                      onChange={handleInputChange}
                      value={pedidoForm.fechavencimiento}
                      min={pedidoForm.fechainicio}
                    />
                    <p style={{ fontSize: "0.7rem", margin: "0px" }}>
                      <strong>
                        Esta fecha está como el mes siguiente por defecto
                      </strong>
                    </p>
                    <Form.Label
                      id="errorFechavencimiento"
                      style={{
                        marginBottom: "0px",
                        fontSize: "0.8rem",
                        color: "red",
                ...
                <Form.Group className="mb-2" controlId="formBasicUrgencia">
                  ...
                  <Form.Control
                    ...
                    onChange={handleInputChange}
                  >
                    <option value="" onClick={() => setUrgencia(0)} hidden>
                      Urgencia
                    </option>
                    <option value="1" onClick={() => setUrgencia(1)}>
                      Ligera
                    </option>
                    <option value="2" onClick={() => setUrgencia(2)}>
                      Moderada
                    </option>
                    <option value="3" onClick={() => setUrgencia(3)}>
                      Extrema
                    </option>
                  </Form.Control>
                  <Form.Label
                ...
                {!stock && (
                  <Form.Group
                    className="mb-2"
                    controlId="formBasicRequestingObra"
                  >
                    <Form.Label
                      className="font-rubik"
                      style={{ fontSize: "0.8rem" }}
                    ...
                    >
                      ...
                      {stocksDisponibles.map(
                        (stock) =>
                          stock.stock &&
                          stock.stock.length > 0 && (
                            <option
                              key={stock.stock[0].id_stock}
                              value={stock.stock[0].id_stock}
                            >
              ...
              <Col xs={12} md={6}>
                <Form.Group
                  style={{ marginTop: "13%" }}
                  className="mb-2"
                  controlId="formBasicObras"
                >
                  <Form.Label
                    className="font-rubik"
                    style={{ fontSize: "0.8rem" }}
                  >
                    Obras que pueden aportar al pedido (*)
                  </Form.Label>
                  <div className="cardscasas">
                    {orderedObras
                      .filter(
                        (obra) => obra.id_obra.id_obra !== pedidoForm.id_obra,
                      )
                      .map((obra) => (
                        <SelectableCard
                          mar={"0.2rem"}
                          ...
                          isSelected={pedidoForm.obras.includes(
                  ...
                  >
                    <Button
                      variant="link"
                      onClick={handleSelectAllObras}
                      style={{ fontSize: "0.8rem", padding: 0 }}
                    >
                      {allSelected
                        ? "Deseleccionar todas las obras"
                        : "Seleccionar todas las obras"}
                    </Button>
                    <Form.Label
                      id="errorObrasRequested"
                      style={{
                        marginBottom: "0px",
                        fontSize: "0.8rem",
                        color: "red",
