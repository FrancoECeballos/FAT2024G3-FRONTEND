...
function PedidoListing({
  ...
  useEffect(() => {
    if (obrasDisponibles && obrasDisponibles.length === 1) {
      if (
        selectedPedido.id_obra &&
        selectedPedido.id_obra.id_obra !== obrasDisponibles[0].id_obra
      ) {
  ...
  const deletePedido = (stock, pedido) => {
    deleteData(`/delete_detalle_pedido/${stock}/${pedido}/`, token)
      .then(() => {
  ...
  const createAportePedido = async (
    pedidoId,
    usuarioId,
    cantidad,
    fechaAportado,
  ) => {
    const pedido = sortedPedidos
      .flatMap((obra) => obra.pedidos)
      .find((pedido) => pedido.id_pedido === pedidoId);
    const cantidadRestante = pedido.cantidad - pedido.progreso;
    if (parseFloat(cantidad) > parseFloat(cantidadRestante)) {
      setError(
        `La cantidad ofrecida no puede exceder la cantidad restante de ${cantidadRestante} ${pedido.id_producto.unidadmedida}`,
      );
      return false;
    ...
    try {
      await postData("/crear_aporte_pedido/", data, token).then(async () => {
        ...
        await crearNotificacion(
          ...
          token,
          "User",
          selectedPedido.id_usuario.id_usuario,
        );
  ...
  const handlePedidoClick = async (pedido) => {
    ...
    if (obrasDisponibles && obrasDisponibles.length > 1) {
      const filteredObras = obrasDisponibles.filter(
        (obra) => pedido.id_obra && pedido.id_obra.id_obra !== obra.id_obra,
      );
    ...
    await fetchData(
      `/producto/categoria/${pedido.id_producto.id_categoria.id_categoria}/`,
      token,
    ).then((result) => {
  ...
  return (
    <div className="pedido-list">
      <h1>
        Viendo{" "}
        {obraSelected
          ? `los pedidos hechos a la obra '${obraSelected.nombre}'`
          : "todos los pedidos"}
      </h1>
      <div className="cardCategori">
        {Array.isArray(sortedPedidos) && sortedPedidos.length > 0 ? (
          sortedPedidos.map((obra) => (
            <GenericAccordion
              titulo={`Pedidos de '${obra.obra.nombre}'`}
              key={obra.obra.id_obra}
                  ...
                  <GenericCard
                    ...
                    descrip1={
                      <>
                        ...
                        {pedido.cantidad} {pedido.id_producto.unidadmedida}
                      </>
                    }
                    descrip2={
                      <>
      ...
      <Modal
        ...
        deleteFunction={() =>
          deletePedido(selectedObra.id_obra, selectedPedido.id_pedido)
        }
                ...
                <GenericCard
                  ...
                  descrip6={
                    <>
                      ...
                      {selectedPedido.id_usuario.nombre}{" "}
                      {selectedPedido.id_usuario.apellido}
                    </>
                  }
                />
                ...
                {shouldShowButtons && (
                  <Form.Group className="mb-2" controlId="formBasicCantidad">
                    ...
                    {selectedObra && (
                      ...
                      >
                        Usted esta aportando desde la obra{" "}
                        <strong>{selectedObra.nombre}</strong>
                      </Form.Label>
                    )}
                      ...
                      <>
                        ...
                        >
                          {obrasDisponibles.map(
                            (obra) =>
                              selectedPedido.id_obra &&
                              selectedPedido.id_obra.id_obra !==
                                obra.id_obra && (
