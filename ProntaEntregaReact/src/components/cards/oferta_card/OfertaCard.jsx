...
const OfertaCard = forwardRef(
  ({ productDefault, user, stock, stocksDisponibles }, ref) => {
    ...
    const formattedNextMonthDate = nextMonth
      .toLocaleDateString("en-GB")
      .split("/")
      .reverse()
      .join("-");
    ...
    const handleFetchProducts = (id_stock, id_categoria) => {
      fetchData(
        `/GetDetallestockproducto_Total/${id_stock}/${id_categoria}/`,
        token,
      )
        .then((result) => {
          setProducts(result);
        ...
        <Card.Body>
          <Form style={{ alignItems: "center" }}>
            <h2 className="centered">
              {productDefault
                ? `Crear Oferta de ${productDefault.nombre}`
                : "Crear Oferta"}
            </h2>
            ...
            <OverlayTrigger
              placement="top"
              overlay={
                <Tooltip style={{ fontSize: "100%" }} id="tooltip-fecha-inicio">
                  La oferta se hará visible este día
                </Tooltip>
              }
            >
            ...
            <OverlayTrigger
              placement="top"
              overlay={
                <Tooltip style={{ fontSize: "100%" }} id="tooltip-fecha-fin">
                  La oferta dejara de ser visible este día
                </Tooltip>
              }
            >
              <Form.Group className="mb-2" controlId="formBasicFechaFin">
                ...
                <Form.Control
                  ...
                  onBlur={handleInputChange}
                  onChange={handleInputChange}
                  defaultValue={formattedNextMonthDate}
                  min={formattedDate}
                />
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => {
                      ...
                      const selectedProduct = products.find(
                        (product) =>
                          product.id_producto === parseInt(e.target.value),
                      );
                      if (selectedProduct) {
