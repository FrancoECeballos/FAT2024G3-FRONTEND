...
function Stock() {
  ...
  const filteredObras = obras.filter((obra) => {
    return (
      ...
      obra.id_obra.id_direccion.localidad
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase()) ||
      obra.id_obra.id_direccion.calle
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase())
    );
  });
  ...
  if (Array.isArray(obras) && obras.length === 1) {
    const obra = obras[0];
    navigate(`/obra/${obra.id_stock}/categoria`, {
      state: { id_stock: `${obra.id_stock}` },
    });
  }
