  ...
  const handleSave = async (cantidad, total, producto) => {
    try {
      if (
        ...
        cantidad <= 0 ||
        isNaN(cantidad) ||
        cantidad > Number.MAX_SAFE_FLOAT
      ) {
      ...
      if (selectedOperacion === "sumar" || producto) {
        ...
        setPopupData({
          title: "Suma exitosa",
          message: `Se sumó el valor de ${cantidad} exitosamente.`,
        });
        ...
        setPopupData({
          title: "Resta exitosa",
          message: `Se restó el valor de ${cantidad} exitosamente.`,
        });
        console.log("Save operation succeeded.");
        return true;
      }
    ...
    return new Promise((resolve, reject) => {
      if (pedidoOrOferta === "pedido") {
        if (pedidoCardRef.current) {
          ...
          postData("/crear_pedido/", pedidoFormWithoutObras, token)
            .then(async (result) => {
              const fechaCreacion = new Date().toISOString().split("T")[0];
              ...
              return Promise.all(obrasPromises).then(async () => {
                setPopupData({
                  title: "Pedido Creado",
                  message: `Se creó el pedido de ${producto[0].nombre} exitosamente.`,
                });
                await reloadData();
                resolve(true);
              });
        ...
        if (ofertaCardRef.current) {
          ...
          postData("/crear_oferta/", ofertaForm, token)
            .then(async () => {
              const fechaCreacion = new Date().toISOString().split("T")[0];
              const producto = await fetchData(
              ...
              return crearNotificacion(dataNotificacion, token).then(
                async () => {
                  setPopupData({
                    title: "Oferta Creada",
                    message: `Se creó la oferta de ${producto[0].nombre} exitosamente.`,
                  });
                  await reloadData();
                  resolve(true);
                },
        ...
        {isLoading ? (
          ...
          <div>
            <Breadcrumb style={{ marginLeft: "8%", fontSize: "1.2rem" }}>
              ...
              <Breadcrumb.Item href={`/obra/${stockId}/categoria`}>
                {currentObra}
              </Breadcrumb.Item>
            ...
            >
              <Modal
                buttonStyle={{ marginTop: "10rem" }}
                openButtonText="Añadir un producto nuevo"
                openButtonWidth="15"
                title="Añadir Producto"
                saveButtonText={selectedCardId !== "New" ? "Agregar" : "Crear"}
                handleShowModal={() =>
                  setDetalle({ id_stock: parseInt(stockId, 10) })
                }
                showPopup={true}
                ...
                content={
                  <div>
                    ...
                    ></GenericAlert>
                    {selectedCardId === "New" ? (
                      <h2 className="centered"> Cree un Producto </h2>
                    ) : (
                    ...
                    >
                      <AutoCompleteSelect
                        ...
                        lists={excludedProducts}
                        selectedKey={selectedCardId}
                        onClick={setSelectedNewProduct}
                        onInputChange={() => {
                          setSelectedCardId();
                          cantidadRef.current = 0;
                        }}
                        ...
                        <>
                          ...
                          <InputGroup className="mb-2">
                            <Form.Control
                              ...
                              type="text"
                              placeholder="Nombre"
                              onBlur={handleProductInputChange}
                            ...
                            <Form.Select
                              name="unidadmedida"
                              onChange={handleProductInputChange}
                              className="unidad-medida-select"
                            >
                ...
                sortedProducts.map((product) => {
                  return (
                    <GenericCard
                      ...
                      descrip2={
                        <>
                          <strong>Cantidad:</strong> {product.total}{" "}
                          ...
                          <Row>
                            <Col xs={12} md={6} style={{ marginTop: "1rem" }}>
                              <Modal
                                ...
                                handleCloseModal={() => setShowAlert(false)}
                            ...
                            <Col xs={12} md={6} style={{ marginTop: "1rem" }}>
                              <Modal
                                tamaño="lg"
                                openButtonText="Crear Pedido / Oferta"
                                openButtonWidth="12"
                                handleCloseModal={() => {
                                  setShowAlert(false);
                                  setPedidoOrOferta("pedido");
                                }}
                                title="Crear Pedido / Oferta"
                                saveButtonText="Crear"
                                handleSave={handleCreatePedidoOrOferta}
                            ...
                            <Col xs={120} style={{ marginTop: "1rem" }}>
                              <OverlayTrigger
                                ...
                                overlay={
                                  <Tooltip
                                    style={{ fontSize: "100%" }}
                                    id={`tooltip-${product.id_producto}`}
                                  >
                              ...
                              >
                                <Icon
                                  ...
                                  onClick={() =>
                                    navigate(
                                      `/obra/${stockId}/categoria/${categoriaID}/producto/${product.id_producto}`,
                                      {
                                        state: {
                                          id_stock: stockId,
                                ...
                                <Col xs={10} style={{ marginTop: "1rem" }}>
                                  ...
                                  >
                                    <Icon
                                      ...
                                      onClick={() =>
                                        setConfirmDelete(product.id_producto)
                                      }
                                    />
                                  </OverlayTrigger>
                                </Col>
