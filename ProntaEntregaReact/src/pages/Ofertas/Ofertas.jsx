...
function Ofertas() {
  ...
  const [terminarOfertaConfirmation, setTerminarOfertaConfirmation] =
    useState(false);
  const [cancelarOfertaConfirmation, setCancelarOfertaConfirmation] =
    useState(false);
  ...
  const filteredOfertas = ofertas.filter((oferta) => {
    return (
      ...
      oferta.id_producto.nombre
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase()) ||
      oferta.id_obra.nombre
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase()) ||
      oferta.id_usuario.nombre
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase())
    );
  });
  ...
  const createAporteOferta = async (
    ofertaId,
    usuarioId,
    obra,
    cantidad,
    ...
    if (parseFloat(cantidad) > parseFloat(cantidadRestante)) {
      setError(
        `La cantidad ofrecida no puede exceder la cantidad restante de ${cantidadRestante} ${oferta.id_producto.unidadmedida}`,
      );
      return false;
    }
    ...
    try {
      await postData("/crear_detalle_oferta/", data, token).then(async () => {
        const fechaCreacion = new Date().toISOString().split("T")[0];
        const ofertaAportada = await fetchData(
          `/oferta_id/${ofertaId}/`,
          token,
        );
        ...
        await crearNotificacion(
          ...
          token,
          "User",
          selectedOferta.id_usuario.id_usuario,
        );
  ...
  const handleDeleteOferta = async (ofertaId) => {
    try {
      ...
      const pendingStock = await fetchData(
        `/stock/${ofertaId.id_obra.id_obra}/`,
        token,
      );
    ...
    <>
          ...
          <>
            ...
            <div className="oferta-list">
              ...
              >
                <Tab
                  key="user_ofertas"
                  eventKey="user_ofertas"
                  title={
                    <strong className="custom-tab-title">Mis Ofertas</strong>
                  }
                  style={{ backgroundColor: "transparent" }}
                >
                  <div className="cardCategori">
                    <h1>Viendo ofertas creadas por usted</h1>
                    {Array.isArray(sortedUserOfertas) &&
                    sortedUserOfertas.length > 0 ? (
                      sortedUserOfertas.map((oferta) => (
                        <div key={oferta.id_oferta}>
                          <GenericCard
                            ...
                            descrip1={
                              <>
                                <strong>Cantidad:</strong> {oferta.progreso} /{" "}
                                {oferta.cantidad}{" "}
                                {oferta.id_producto.unidadmedida}
                              </>
                            }
                            descrip2={
                              <>
                                <strong>Obra:</strong> {oferta.id_obra.nombre}{" "}
                                <strong>Usuario:</strong>{" "}
                                {oferta.id_usuario.nombre}{" "}
                                {oferta.id_usuario.apellido}
                              </>
                            }
                            descrip3={
                              <>
                                <strong>Estado:</strong>{" "}
                                {oferta.id_estadoOferta.nombre}
                              </>
                            }
                            descrip4={
                              <>
                ...
                <Tab
                  ...
                  eventKey="ofertas"
                  title={
                    <span className="custom-tab-title">Ofertas para mi</span>
                  }
                  style={{ backgroundColor: "transparent" }}
                >
                  <div className="cardCategori">
                    <h1>Viendo ofertas disponibles</h1>
                    {Array.isArray(sortedOfertas) &&
                            ...
                            onClick={() => {
                              ...
                              if (obras.length > 1) {
                                const filteredObras = obras.filter(
                                  (obra) =>
                                    oferta.id_obra &&
                                    oferta.id_obra.id_obra !== obra.id_obra,
                                );
      ...
      {selectedOferta && (
        <Modal
          ...
          handleCloseModal={() => {
            setShowTakeOfertaModal(false), setSelectedOferta(null);
          }}
          saveButtonShown={
            !(
              obras.length === 1 &&
              obras[0].id_obra === selectedOferta.id_obra.id_obra
            )
          }
          title="Tomar Oferta"
          handleSave={() =>
            createAporteOferta(
              selectedOferta.id_oferta,
              user.id_usuario,
              selectedObra,
              cantidad,
          ...
          content={
            <div>
              <GenericCard
                ...
                descrip2={
                  <>
                    <strong>Obra:</strong> {selectedOferta.id_obra.nombre}
                  </>
                }
                descrip3={
                  <>
                    <strong>Usuario:</strong> {selectedOferta.id_usuario.nombre}{" "}
                    {selectedOferta.id_usuario.apellido}
                  </>
                }
                descrip4={
                  <>
                    <strong>Estado:</strong>{" "}
                    {selectedOferta.id_estadoOferta.nombre}{" "}
                    <strong>Cantidad:</strong> {selectedOferta.cantidad}{" "}
                    {selectedOferta.id_producto.unidadmedida}
                  </>
                }
                descrip5={
                  <>
                    <strong>Fecha Vencimiento:</strong>{" "}
                    {selectedOferta.fechavencimiento
                      ? selectedOferta.fechavencimiento
                          .split("-")
                          .reverse()
                          .join("/")
              ...
              {!(
                ...
                <>
                  ...
                  {obras.length === 1 ? (
                    ...
                    >
                      Usted esta tomando esta oferta para la obra{" "}
                      <strong>{obras[0].nombre}</strong>
                    </Form.Label>
                  ) : (
                    <>
                      <Form.Label
                      ...
                      <Form.Control
                        ...
                        onChange={(event) => {
                          setSelectedObra(
                            obras.find(
                              (obra) =>
                                obra.id_obra === Number(event.target.value),
                            ),
                          );
                        }}
                      >
                        {obras
                          .filter(
                            (obra) =>
                              selectedOferta.id_obra &&
                              selectedOferta.id_obra.id_obra !== obra.id_obra,
                          )
                          .map((obra) => (
            ...
            {selectedAporte && (
              <GenericCard
                ...
                descrip1={
                  <>
                    <strong>Obra: </strong>
                    {selectedAporte.id_obra.nombre}
                  </>
                }
                descrip2={
                  <>
                    <strong>Usuario: </strong>
                    {selectedAporte.id_usuario.nombre}{" "}
                    {selectedAporte.id_usuario.apellido}
                  </>
                }
              />
            )}
      ...
      />
      <ConfirmationModal
        Open={terminarOfertaConfirmation}
        BodyText="Al terminar la oferta se donaran los productos tomados."
        onClickConfirm={() => handleEndOferta(selectedOferta.id_oferta)}
        onClose={() => setTerminarOfertaConfirmation(false)}
      />
      <ConfirmationModal
        Open={cancelarOfertaConfirmation}
        BodyText="Si cancelas la oferta no se donaran los productos tomados ¿Está seguro?"
        onClickConfirm={() => handleDeleteOferta(selectedOferta)}
        onClose={() => setCancelarOfertaConfirmation(false)}
      />
      ...
      {user.is_superuser && (
        ...
        >
          <SendButton
            onClick={() => navigate(`/informe_ofertas`)}
            text="Ver Informe de Ofertas"
            wide="14"
          />
        </div>
      )}
