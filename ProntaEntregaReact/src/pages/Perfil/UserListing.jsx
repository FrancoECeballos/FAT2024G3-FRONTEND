...
function UserListing() {
  ...
  useEffect(() => {
    const fetchObras = async () => {
      try {
        ...
        console.error(
          "Hubo un error al obtener las obras y los usuarios",
          error,
        );
      } finally {
        setIsLoading(false);
    ...
    fetchData(`/user/obra/null/`, token)
      .then((result) => {
  ...
  const filteredObras = obras.filter((obra) => {
    ...
    const usuariosMatches = obra.usuarios?.some(
      (usuario) =>
  ...
  const sortedObras = [...filteredObras].sort((a, b) => {
    ...
    if (a[orderCriteria]?.toLowerCase() < b[orderCriteria]?.toLowerCase())
      return -1;
    if (a[orderCriteria]?.toLowerCase() > b[orderCriteria]?.toLowerCase())
      return 1;
    return 0;
  ...
  const filteredUsuariosSinObra = usuariosSinObra.filter(
    (usuario) =>
      usuario.nombre?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      usuario.apellido?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      usuario.email?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <div style={{ marginTop: "1rem" }}>
                ...
                >
                  {obra.usuarios.map((usuario) => (
                    <div
                      ...
                      style={{ flex: "", boxSizing: "border-box" }}
                    >
                      <LittleCard
                        foto={usuario.imagen}
                        ...
                        onSelect={() =>
                          navigate(`/perfil/micuenta`, {
                            state: { user_email: usuario.email },
                          })
                        }
                      />
                    </div>
          ...
          {filteredUsuariosSinObra.length > 0 && (
            <div key="NoObra" style={{ marginBottom: "0.1rem" }}>
              <GenericAccordion wide={"80%"} titulo="Usuarios sin Obra">
                <div
                  style={{
                    display: "flex",
                    flexWrap: "wrap",
          ...
          {sortedObras.length === 0 && filteredUsuariosSinObra.length === 0 && (
            <p style={{ marginLeft: "7rem", marginTop: "1rem" }}>
              No hay obras y/o usuarios disponibles.
            </p>
          )}
        </div>
      </div>
