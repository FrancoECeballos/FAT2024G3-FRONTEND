...
function MiCuenta() {
    ...
    const updateUser = async () => {
      try {
        ...
        if (location.state && location.state != null) {
          const viewedUserResult = await fetchData(
            `/user/${location.state.user_email}`,
          );
          setUser({
            viewedUser: viewedUserResult,
            viewingUser: result,
  ...
  return (
    <div
      style={{ backgroundColor: "#ECECEC", overflowX: "hidden", width: "100%" }}
    >
      <FullNavbar />
      {isLoading ? (
        <Loading />
