...
function ObrasUsers() {
            ...
            if (result.is_superuser) {
                ...
                fetchData(`/user/obrasEmail/${email}/`, token).then((result) => {
                    ...
                    Promise.all(obraPromises).then(obras => {
                        ...
                        const obrasWithModerator = uniqueObras.map(obra => {
                            const originalObra = result.find(o => o.id_obra === obra.id_obra);
                            return {
                                ...obra,
                                isModerator: originalObra.id_tipousuario >= 2
                            };
                        });
                
                        console.log("Fetched Obras:", obrasWithModerator);
                        setObras(obrasWithModerator);
                    }).catch(error => {
    ...
    if (Array.isArray(sortedObras) && sortedObras.length === 1) {
        ...
        navigate(`/selecteduser/${obra.id_obra}`, {state: {id_obra: `${obra.id_obra}`}});
    }
    ...
    return (
        <div>
            ...
            <h4 style={{marginLeft: '8%', color: 'grey'}}>Usuarios</h4>
                <SearchBar onSearchChange={handleSearchChange} onOrderChange={setOrderCriteria} filters={filters}/>
                <div className='cardstock'>
                {Array.isArray(sortedObras) && sortedObras.length > 0 ? (
                    sortedObras.map(obra => (
                        <GenericCard
                            onClick={() => navigate(`/selecteduser/${obra.id_obra}`, {state: {id_obra: `${obra.id_obra}`}})} 
                            key={obra.id_obra}
                            foto={obra.imagen}
                            titulo={obra.nombre}
