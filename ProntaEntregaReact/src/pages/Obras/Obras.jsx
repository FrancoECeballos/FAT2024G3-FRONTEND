...
function Stock() {
    ...
    fetchUser(navigate)
      .then((result) => {
        if (result.is_superuser) {
          ...
          fetchData(`/user/obrasEmail/${result.email}/`, token)
            .then((result) => {
              setObras(result);
            })
            .catch((error) => {
        ...
        img.onload = () => {
          ...
          canvas.toBlob((blob) => {
            const file = new File([blob], "WhiteLogo.png", {
              type: "image/png",
            });
  ...
  const validateForm = (name, value) => {
    ...
    const localidadValue =
      typeof obraForm.id_direccion.localidad === "object"
        ? obraForm.id_direccion.localidad.label
        : obraForm.id_direccion.localidad;
  ...
  const handleInputChange = (event) => {
    ...
    if (name === "localidad" || name === "calle" || name === "numero") {
      if (name === "numero") {
        ...
        setObraForm({
          ...obraForm,
          id_direccion: { ...obraForm.id_direccion, [name]: parsedValue },
        });
      } else {
        setObraForm({
          ...obraForm,
          id_direccion: { ...obraForm.id_direccion, [name]: value },
        });
      }
        ...
        {isLoading ? (
          ...
          <>
            ...
            {isAdmin && (
              ...
              >
                <Modal
                  openButtonText="Crear una nueva obra"
                  openButtonWidth="12"
                  title="Nueva Obra"
                  saveButtonText="Crear"
                  saveButtonEnabled={isFormValid}
                  handleSave={() => handleSaveObra()}
                  handleShowModal={() => {
                    setObraForm({
                      nombre: "",
                      descripcion: "",
                    ...
                    <div>
                      ...
                      <Form.Group controlId="direccion">
                        ...
                        <SelectLocalidad
                          ...
                          onSelect={(label) => {
                            handleInputChange({
                              target: { name: "localidad", value: label },
                            });
                          ...
                          <>
                            ...
                            <Form.Group controlId="direccion">
                              ...
                              <SelectLocalidad
                                ...
                                onChange={(value) => {
                                  const isValid =
                                    SelectLocalidad.Localidades.some(
                                      (localidad) => localidad.label === value,
                                    );
                                  if (!isValid) {
                                    handleInputChange({
