...
function ObrasAutos() {
    ...
    fetchUser()
      .then((result) => {
        ...
        if (result.is_superuser) {
          ...
          fetchData(`/user/obrasEmail/${email}/`, token)
            .then((result) => {
              const obraIds = result.map((obra) => obra.id_obra);
              const obraPromises = obraIds.map((id) =>
                fetchData(`/obra/${id}`, token),
              );
              Promise.all(obraPromises)
                .then((obras) => {
                  const uniqueObras = Array.from(
                    new Set(obras.flat().map((obra) => JSON.stringify(obra))),
  ...
  if (Array.isArray(obras) && obras.length === 1) {
    const obra = obras[0];
    navigate(`/vehiculos/${obra.id_obra}`, {
      state: { id_obra: `${obra.id_obra}` },
    });
  }
